...
//                        $weeklySalary -= $underTimeDeduction;
                        ...
                        if ($employeePayrollData[0]->salary_group_id) {
                            ...
                            $totalAllowance += $totalIncome;
                            $totalDeduction += $total_deduction;
                            Log::info('total income component '.$totalIncome);
                            $monthSalary += $totalIncome;
                            Log::info('month salary after income component '.$monthSalary);
                            Log::info('total deduction component '.$total_deduction);
                            $monthSalary -= $total_deduction;
                            Log::info('month salary after component deduction '.$monthSalary);
                        ...
                        if (count($additionalComponents) > 0) {
                            ...
                            foreach ($additionalSalaryComponents as $component) {
                                ...
                                if ($component['type'] == 'deductions') {
                                    Log::info('additional deduction component '.$component['monthly']);

                                    $monthSalary -= $component['monthly'];
                                    Log::info('month salary after additional deduction component '.$monthSalary);
                                }
                                if ($component['type'] == 'earning') {
                                    Log::info('additional income component '.$component['monthly']);
                                    $monthSalary += $component['monthly'];
                                    Log::info('month salary after additional income component '.$monthSalary);
                                }
                        ...
                        if(count($bonus) > 0){
                            Log::info('bonus '.$bonus['amount']);
                            $bonusAmount = $bonus['amount'];
                            $monthSalary += $bonusAmount;
                            Log::info('month salary after bonus '.$monthSalary);
                            if ($filterData['include_tds'] == 1) {
                                $bonusTax = $bonus['tax'];
                                $monthSalary -= $bonusTax;
                                Log::info('bonus tax '.$bonus['tax']);
                                Log::info('month salary after bonus tax '.$monthSalary);
                            }
                        }

...
class AppHelper
{
   ...
   public static function attendanceMethod()
    ...
    public static function allowMultipleLeaveRequest(): bool
    {
        $slug = 'allow-multiple-leave-request';
        return AppSetting::where('slug', $slug)->where('status', 1)->exists();
    }
...
//    }
    ...
    public static function getEmployeeCode(){
        ...
        $code = $prefix . str_pad(1, 4, '0', STR_PAD_LEFT);

        if(isset($user) && $user->employee_code){
            $codeNumber = explode('-', $user->employee_code);
            $lastSegment = end($codeNumber);

            if ($lastSegment !== false && is_numeric($lastSegment)) {
                $codeId = (int) $lastSegment + 1;
                $code = $prefix . str_pad($codeId, 4, '0', STR_PAD_LEFT);
            }

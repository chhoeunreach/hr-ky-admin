...
class UserRepository
{
    ...
    public function getAllUsers($filterParameters, $select = ['*'], $with = [])
    {
        ...
        $userList =  User::with($with)
            ...
            if ($requestedPerPage === 'all') {
                $totalRecord = (clone $userList)->count();
                return $userList->paginate($totalRecord > 0 ? $totalRecord : 1);
            }
            ...
            return $userList->paginate($perPage);
    }
    ...
    {
        return User::where('employee_code','like', '%'.$employeeCode)->first();
    }

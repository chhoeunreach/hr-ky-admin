<script>
    $(document).ready(function () {
        ...
        $('#notice').select2({
            placeholder:"<?php echo e(__('index.select_notice_receiver')); ?>"
        });
        ...
        $('.toggleStatus').change(function (event) {
            ...
            Swal.fire({
                title: '<?php echo app('translator')->get('index.confirm_change_notice_status'); ?>',
                showDenyButton: true,
        ...
        $('.delete').click(function (event) {
            ...
            Swal.fire({
                title: '<?php echo app('translator')->get('index.confirm_delete_notice'); ?>',
                showDenyButton: true,
                confirmButtonText: `<?php echo app('translator')->get('index.yes'); ?>`,
                denyButtonText: `<?php echo app('translator')->get('index.no'); ?>`,
                padding:'10px 50px 10px 50px',
        ...
        $('.sendNotice').click(function (event) {
            ...
            let href = $(this).data('href');
            Swal.fire({
                title: '<?php echo app('translator')->get('index.confirm_send_notice'); ?>',
                showDenyButton: true,
                confirmButtonText: `<?php echo app('translator')->get('index.yes'); ?>`,
                denyButtonText: `<?php echo app('translator')->get('index.no'); ?>`,
                padding:'10px 50px 10px 50px',
        ...
        $('body').on('click', '#showNoticeDescription', function (event) {
            ...
            $.get(url, function (data) {
                ...
                $('.modal-title').html(<?php echo json_encode(__('index.notice_detail_modal_title', ['title' => '']), 512) ?>+': ' + data.data.title );
    ...
    $(document).ready(function () {
        ...
        const filterEmployeeIds = JSON.parse('<?php echo json_encode($filterParameters['notice_receiver'] ?? []); ?>');
        let receiverUserIds = [];
        ...
        } else if (<?php echo isset($noticeDetail) && !empty($receiverUserIds) ? 'true' : 'false'; ?>) {
            // Use form employee IDs if no filter params
            receiverUserIds = <?php echo isset($noticeDetail) && !empty($receiverUserIds) ? json_encode($receiverUserIds) : '[]'; ?>.map(String);
        }
        ...
        const loadEmployees = async (selectedBranchId) => {
            ...
            try {
                const response = await $.ajax({
                    type: 'GET',
                    url: `<?php echo e(url('admin/get-branch-employee-data')); ?>/${selectedBranchId}`,
                });
                ...
                if (response.users && response.users.length > 0) {
                    ...
                    $('#notice').append('<option disabled><?php echo e(__("index.employee_not_found")); ?></option>');
                }

            } catch (error) {
                console.error('Error loading data:', error);
                $('#notice').append('<option disabled><?php echo e(__("index.error_loading_employees")); ?></option>');
            }
        };
...
<?php /**PATH /var/www/hr-ky-admin1/resources/views/admin/notice/common/scripts.blade.php ENDPATH**/ ?>
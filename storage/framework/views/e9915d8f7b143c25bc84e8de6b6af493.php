...
<script src="<?php echo e(asset('assets/jquery-validation/jquery.validate.min.js')); ?>"></script>
<script src="<?php echo e(asset('assets/jquery-validation/additional-methods.min.js')); ?>"></script>
...
<script>
    $(document).ready(function () {
        ...
        $('.changePassword').click(function (event) {
            ...
            $('.modal-title').html('<?php echo e(__('index.user_change_password')); ?>');
        ...
        $('.toggleStatus').change(function (event) {
            ...
            Swal.fire({
                title: '<?php echo e(__('index.confirm_change_status')); ?>',
                showDenyButton: true,
        ...
        $('.toggleHolidayCheckIn').change(function (event) {
            ...
            Swal.fire({
                title: '<?php echo e(__('index.confirm_change_holiday_checkin')); ?>',
                showDenyButton: true,
                confirmButtonText: `<?php echo e(__('index.yes')); ?>`,
                denyButtonText: `<?php echo e(__('index.no')); ?>`,
                padding: '10px 50px 10px 50px',
        ...
        $('.deleteEmployee').click(function (event) {
            ...
            Swal.fire({
                title: '<?php echo e(__('index.confirm_delete_employee')); ?>',
                showDenyButton: true,
                confirmButtonText: `<?php echo e(__('index.yes')); ?>`,
                denyButtonText: `<?php echo e(__('index.no')); ?>`,
                padding: '10px 50px 10px 50px',
        ...
        $('.forceLogOut').click(function (event) {
            ...
            let href = $(this).data('href');
            Swal.fire({
                title: '<?php echo e(__('index.confirm_force_logout')); ?>',
                showDenyButton: true,
                confirmButtonText: `<?php echo e(__('index.yes')); ?>`,
                denyButtonText: `<?php echo e(__('index.no')); ?>`,
                padding: '10px 50px 10px 50px',
        ...
        $('.changeWorkPlace').click(function (event) {
            ...
            let href = $(this).data('href');
            Swal.fire({
                title: '<?php echo e(__('index.confirm_change_workplace')); ?>',
                showDenyButton: true,
                confirmButtonText: `<?php echo e(__('index.yes')); ?>`,
                denyButtonText: `<?php echo e(__('index.no')); ?>`,
                padding: '10px 50px 10px 50px',
    ...
    $('#employeeDetail').validate({
        ...
        messages: {
            name: {
                required: "<?php echo e(__('index.enter_name')); ?>",
            },
            address: {
                required: "<?php echo e(__('index.enter_address')); ?>"
            },
            email: {
                required: "<?php echo e(__('index.enter_valid_email')); ?>"
            },
            role_id: {
                required: "<?php echo e(__('index.select_role')); ?>"
            },
            username: {
                required: "<?php echo e(__('index.enter_username')); ?>"
            }
        },
        errorElement: 'span',
    ...
    $(document).ready(function () {
        const loadDepartmentsAndOfficeTime = async () => {
            ...
            let departmentId = "<?php echo e($userDetail->department_id ?? $filterParameters['department_id'] ?? old('department_id')); ?>";
            let officeTimeId = "<?php echo e(isset($userDetail) ? $userDetail['office_time_id'] : old('office_time_id')); ?>";
            ...
            try {
                const response = await $.ajax({
                    type: 'GET',
                    url: `<?php echo e(url('admin/transfer/get-user-transfer-branch-data')); ?>/${selectedBranchId}`,
                });
                ...
                // Departments
                if (!departmentId) {
                    $('#department').append('<option disabled selected><?php echo e(__('index.select_department')); ?></option>');
                }
                if (response.departments && response.departments.length > 0) {
                    ...
                    $('#department').append('<option disabled><?php echo e(__("index.no_department_found")); ?></option>');
                }

                // Office Times
                if (!officeTimeId) {
                    $('#officeTime').append('<option value="" selected><?php echo e(__('index.select_office_time')); ?></option>');
                }
                if (response.officeTimes && response.officeTimes.length > 0) {
                    ...
                    $('#officeTime').append('<option disabled><?php echo e(__("index.office_time_not_found")); ?></option>');
                }
            } catch (error) {
                $('#department').append('<option disabled><?php echo e(__("index.error_loading_departments")); ?></option>');
                $('#officeTime').append('<option disabled><?php echo e(__("index.error_loading_office_times")); ?></option>');
            }
        };

        const loadSupervisorAndPosts = async () => {
            const selectedDepartmentId = $('#department').val(); // Changed selector to #department
            let supervisorId = "<?php echo e(isset($userDetail) ? $userDetail['supervisor_id'] : old('supervisor_id')); ?>";
            let employeeId = "<?php echo e(isset($userDetail) ? $userDetail['id'] : ''); ?>";
            let postId = "<?php echo e(isset($userDetail) ? $userDetail['post_id'] : old('post_id')); ?>";

            if (!selectedDepartmentId) return;

            try {
                const response = await fetch(`<?php echo e(url('admin/transfer/get-user-transfer-department-data')); ?>/${selectedDepartmentId}`, {
                    method: 'GET',
                    headers: {
                ...
                // Supervisors
                if (!supervisorId) {
                    $('#supervisor').append('<option value="" selected><?php echo e(__('index.select_supervisor')); ?></option>');
                }
                if (data.supervisors && data.supervisors.length > 0) {
                    ...
                    $('#supervisor').append('<option disabled><?php echo e(__("index.no_employees_found")); ?></option>');
                }

                // Posts
                if (!postId) {
                    $('#post').append('<option value="" selected><?php echo e(__('index.select_option')); ?></option>');
                }
                if (data.posts && data.posts.length > 0) {
                    ...
                    $('#post').append('<option disabled><?php echo e(__("index.no_posts_found")); ?></option>');
                }
            } catch (error) {
                $('#supervisor').append('<option disabled><?php echo e(__("index.error_loading_employees")); ?></option>');
                $('#post').append('<option disabled><?php echo e(__("index.error_loading_posts")); ?></option>');
            }
        };
        ...
        const loadLeaveTypes = async () => {
            ...
            try {
                const response = await $.ajax({
                    url: `<?php echo e(url('admin/leaves/get-gender-leave-types')); ?>/${branch}/${gender}`,
                    method: 'GET',
                });
                ...
                <tr>
                    ...
                    <td>
                        <input class="me-1 is-active-checkbox" type="checkbox"
                               name="is_active[${index}]" value="1"><?php echo e(__('index.is_active')); ?>

                        </td>
                    </tr>`;
                    });
                } else {
                    tableBody = '<tr><td colspan="3"><?php echo e(__("index.no_leave_types_found")); ?></td></tr>';
                }
                ...
                $('#leave-types-table').html('<tr><td colspan="3"><?php echo e(__("index.error_loading_leave_types")); ?></td></tr>');
            }
        };
...
<?php /**PATH /var/www/hr-ky-admin1/resources/views/admin/employees/common/scripts.blade.php ENDPATH**/ ?>
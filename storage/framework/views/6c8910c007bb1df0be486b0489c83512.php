...
<script>
    $('document').ready(function(){
        ...
        $('body').on('click', '.delete', function (event) {
            ...
            Swal.fire({
                title: '<?php echo e(__('index.delete_confirmation')); ?>',
                showDenyButton: true,
...
<script>
    $(function () {
        ...
        $(document).on('change', '.staff-select', function () {
            ...
            if (roleId) {
                $userDropdown.html('<option selected disabled><?php echo e(__("index.select_employee")); ?></option>');
    ...
    $(document).ready(function () {
        ...
        const filterDepartmentIds = <?php echo json_encode($filterParameters['department_id'] ?? []); ?> || [];
        const formDepartmentIds = <?php echo isset($departmentId) ? json_encode($departmentId) : '[]'; ?>;
        const departmentIds = filterDepartmentIds.length > 0 ? filterDepartmentIds.map(String) : formDepartmentIds.map(String);
        let leaveTypeId = "<?php echo e($leaveApprovalDetail->leave_type_id ?? $filterParameters['leave_type_id'] ?? ''); ?>";
        ...
        const loadLeaveTypeAndDepartments = async (selectedBranchId) => {
            ...
            try {
                const response = await $.ajax({
                    type: 'GET',
                    url: `<?php echo e(url('admin/get-branch-leave-data')); ?>/${selectedBranchId}`,
                });
                console.log('AJAX response:', response);

                $('#related').empty();
                $('#departments').empty().append('<option value="all"><?php echo e(__("index.select_all")); ?></option>');

                if (!leaveTypeId) {
                    $('#related').append('<option value="" disabled selected><?php echo e(__('index.select_leave_type')); ?></option>');
                }
                if (response.types && response.types.length > 0) {
                    ...
                    $('#related').append('<option disabled><?php echo e(__("index.leave_type_not_found")); ?></option>');
                }
                ...
                if (response.departments && response.departments.length > 0) {
                    ...
                    $('#departments').append('<option disabled><?php echo e(__("index.no_department_found")); ?></option>');
                }
                ...
                $('#related').append('<option disabled><?php echo e(__("index.error_loading_leave_types")); ?></option>');
                $('#departments').append('<option disabled><?php echo e(__("index.error_loading_department")); ?></option>');
...
<?php /**PATH /var/www/hr-ky-admin1/resources/views/admin/leaveApproval/common/scripts.blade.php ENDPATH**/ ?>